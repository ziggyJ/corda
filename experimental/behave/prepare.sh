...
cd ../..
./gradlew clean :node:capsule:buildCordaJar :finance:contracts:jar :finance:workflows:jar
cp -v $(ls -t node/capsule/build/libs/corda-*.jar | head -1) ${CORDA_DIR}/corda.jar
...
cp -v $(ls -t tools/bootstrapper/build/libs/*.jar | head -1) ${CORDA_DIR}/network-bootstrapper.jar
