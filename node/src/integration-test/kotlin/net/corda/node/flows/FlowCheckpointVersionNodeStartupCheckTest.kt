...
class FlowCheckpointVersionNodeStartupCheckTest {
    ...
    fun `restart node successfully with suspended flow`() {
        ...
        val cordapps = setOf(TestCorDapp.Builder("testJar", "1.0").plusClasses(classes).build())
    ...
    fun `restart nodes with incompatible version of suspended flow due to different jar name`() {
        ...
        assertNodeRestartFailure(
                emptySet(),
                setOf(TestCorDapp.Builder("testJar", "1.0").plusClasses(classes).build()),
                setOf(TestCorDapp.Builder("testJar2", "1.0").plusClasses(classes).build()),
                false,
    ...
    fun `restart nodes with incompatible version of suspended flow`() {
        ...
        assertNodeRestartFailure(
                emptySet(),
                setOf(TestCorDapp.Builder("testJar", "1.0").plusClasses(classes).build()),
                setOf(TestCorDapp.Builder("testJar", "1.0").plusClasses(classes).plusClasses(net.test.cordapp.v1.SendMessageFlow::class.java).build()),
                false,
