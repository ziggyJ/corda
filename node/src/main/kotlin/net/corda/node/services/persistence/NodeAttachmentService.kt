...
class NodeAttachmentService(
    ...
    override fun importAttachment(jar: InputStream): AttachmentId {
        return withContractsInJar(jar) { contracts, stream ->
            import(stream, UNKNOWN_UPLOADER, null, contracts)
        }
    }
    ...
    override fun importAttachment(jar: InputStream, uploader: String, filename: String?): AttachmentId {
        ...
        return withContractsInJar(jar) { contracts, stream ->
            import(stream, uploader, filename, contracts)
        }
    }

    override fun privilegedImportAttachment(jar: InputStream, uploader: String, filename: String?): AttachmentId {
        return import(jar, uploader, filename, emptyList())
    }

    override fun privilegedImportContractAttachment(jar: InputStream, uploader: String, filename: String?, contractClassNames: List<String>): AttachmentId {
        return import(jar, uploader, filename, contractClassNames)
    }

    override fun privilegedImportOrGetContractAttachment(jar: InputStream, uploader: String, filename: String?, contractClassNames: List<String>): AttachmentId {
        return try {
            import(jar, uploader, filename, contractClassNames)
    ...
    override fun privilegedImportOrGetAttachment(jar: InputStream, uploader: String, filename: String?): AttachmentId {
        return privilegedImportOrGetContractAttachment(jar, uploader, filename, emptyList())
    }
    ...
    private fun import(inputStream: InputStream, uploader: String?, filename: String?, contractClassNames: List<String> ): AttachmentId {
        ...
        val duplicate = database.transaction {
            if (!hasAttachment(id)) {
            ...
            if (isUploaderTrusted(uploader)) {
                ...
                return@transaction !differentUploader
            }
            true
        }
