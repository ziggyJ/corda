...
interface  ServiceHub : ServicesForResolution {
    ...
    fun signInitialTransaction(builder: TransactionBuilder, publicKey: PublicKey): SignedTransaction {
        val actualPublicKey = findFirstLeafKey(publicKey)
        return signInitialTransaction(builder, actualPublicKey, SignatureMetadata(myInfo.platformVersion, Crypto.findSignatureScheme(actualPublicKey).schemeNumberID))
    }
    /**
     * Helper method to construct an initial partially signed transaction from a TransactionBuilder
    ...
    fun createSignature(signedTransaction: SignedTransaction, publicKey: PublicKey): TransactionSignature {
        val actualPublicKey = findFirstLeafKey(publicKey)
        return createSignature(signedTransaction, publicKey, SignatureMetadata(myInfo.platformVersion, Crypto.findSignatureScheme(actualPublicKey).schemeNumberID))
    }
    ...
    fun createSignature(filteredTransaction: FilteredTransaction, publicKey: PublicKey): TransactionSignature {
        val actualPublicKey = findFirstLeafKey(publicKey)
        return createSignature(filteredTransaction, actualPublicKey, SignatureMetadata(myInfo.platformVersion, Crypto.findSignatureScheme(actualPublicKey).schemeNumberID))
    }
